...
import { getAddressDefault, getAddressDetail, getstore_busines} from '../../api/user.js';
import { bind_tg_code_index } from '../../api/api.js';
...
*/
  userNameInput: function (e) {
    this.setData({ tg_code: e.detail.value });
  },
  finsha:function(e){
  var  that = this
  var data={
    tg_code: that.data.tg_code
  }
    bind_tg_code_index(data).then(res => {
      console.log(that.data.tg_code )
      console.log(that.data.tg_code2)
      if (that.data.tg_code != that.data.tg_code2) {
        that.setData({
          active2: false,
          spread_uid: 1
        })
      }else{
        return app.Tips({ title: '请填写你上级的邀请码ID' });
      }
 ...
 */
  ...
  getConfirm:function(){
    ...
    orderConfirm(this.data.cartId).then(res=>{
      that.setData({
        ...
        spread_uid: res.data.userInfo.spread_uid,
        tg_code2: res.data.userInfo.  tg_code,
      ...
      console.log(that.data.spread_uid )
  ...
  */
  ...
  SubOrder:function(e){
    var that = this, data={};
    if (this.data.spread_uid == '' || this.data.spread_uid == 0 ){
